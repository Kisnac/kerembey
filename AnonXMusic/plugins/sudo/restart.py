...
async def restart_(_, message):
    response = await message.reply_text("ʏᴇɴɪᴅᴇɴ ʙᴀsʟᴀᴛɪʟɪʏᴏʀ...")
    ac_chats = await get_active_chats()
    for x in ac_chats:
        try:
            await app.send_message(
                chat_id=int(x),
                text=f"{app.mention} ʏᴇɴɪᴅᴇɴ ʙᴀsʟᴀᴛɪʟɪʏᴏʀ...\n\n𝟷𝟻-𝟸𝟶 sᴀɴıʏᴇ sᴏɴʀᴀ ʙᴀsʟᴀᴛᴀʙɪʟɪʀsɪɴ.",
            )
            await remove_active_chat(x)
            await remove_active_video_chat(x)
        except:
    ...
    await response.edit_text(
        "» ʏᴇɴɪᴅᴇɴ ʙᴀsʟᴀᴛᴍᴀ ɪsʟᴇᴍɪ ʙɪᴛᴛɪ 𝟻-𝟷𝟶 sᴀɴɪʏᴇ sᴏɴʀᴀ ᴛᴜᴍ ᴏᴢᴇʟʟɪᴋʟᴇʀ ᴄᴀʟɪsɪᴄᴀᴋᴛɪʀ..."
    )
    os.system(f"kill -9 {os.getpid()} && bash start")
